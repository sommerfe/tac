...
class TeamStats:
    def __init__(self, name, wins=0, losses=0, ties=0, points_for=0, points_against=0):
        self.team_id = ''
        self.team_name = name
        self.wins = wins
        self.losses = losses
        ...
        self.win_percentage = self.wins / (self.wins + self.losses + self.ties)
        self.points_difference = self.points_for - self.points_against
        self.points_difference_percentage = self.points_difference / (self.points_for + self.points_against)
        self.games: list[Game] = []

    def add_game (self, game: Game):
        self.games.append(game)

        if game.winner == self.team_name:
            self.wins += 1
